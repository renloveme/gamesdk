...
import com.example.mygamesdk.tool.GameSmsContent;
...
final class GamePhoneVerifyCodeActivity extends ActivityUI {
	...
	public LinearLayout onCreate(final Activity activity) {
		...
		LinearLayout.LayoutParams txvwParams = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT,0.5f);
		...
		final GameSdkButton verifySubBtn = new GameSdkButton(activity, asset.getLangProperty(activity, "phone_vcode_post")) {
			...
			public void click(View view) {
				...
				try {
					...
					//填写验证码和密码之后点击
					Dispatcher.getInstance().phoneResetPassword(activity, mobile, vcodeEdtx.getText().toString(),passwordEdtx.getText().toString(),status);
				} catch (Exception e) {
					GameSdkLog.getInstance().e("[手机登录验证码] 提交异常：", e);
				}
			}
		};
		verifySubBtn.setMargins(0, (int) (GameSdkConstants.DEVICE_INFO.windowHeightPx * 0.05), 0, (int) (GameSdkConstants.DEVICE_INFO.windowHeightPx * 0.03));
		addViewListener(activity, mobile, sendedPhoneTxvw, vcodeEdtx);
	...
	private void addViewListener(final Activity activity, final String mobile, TextView sendedPhoneTxvw, final EditText vcodeEdtx) {
		...
		reGetVcodeTxvw.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				try {
					//重发验证码的接口
					Dispatcher.getInstance().reSendPhoneResetPwdVcode(activity, mobile, timer);
				} catch (Exception e) {
